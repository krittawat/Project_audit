    ...
    FileInput.prototype = {
        ...
        _init: function (options, refreshMode) {
            ...
            self.$errorContainer = $h.getElement(options, 'elErrorContainer',
                self.$previewContainer.find('.input-error'));
                ...
                init: function (id, f, index) {
                    ...
                    if (self.showPreview) {
                        ...
                        if (rm.$thumb && rm.$thumb.length) {
                            ...
                            rm.$btnDelete = rm.$thumb.find('.-remove');
                        }
                    }
        ...
        _initTemplateDefaults: function () {
            ...
            tActions = '<div class="file-actions">\n' +
                ...
                '<div class="clearfix"></div>';
            //noinspection HtmlUnknownAttribute
            tActionDelete = '<button type="button" class="-remove {removeClass}" ' +
                'title="{removeTitle}" {dataUrl}{dataKey}>{removeIcon}</button>\n';
            tActionUpload = '<button type="button" class="-upload {uploadClass}" title="{uploadTitle}">' +
                '{uploadIcon}</button>';
            tActionDownload = '<a class="-download {downloadClass}" title="{downloadTitle}" ' +
                'href="{downloadUrl}" download="{caption}" target="_blank">{downloadIcon}</a>';
            tActionZoom = '<button type="button" class="-zoom {zoomClass}" ' +
                'title="{zoomTitle}">{zoomIcon}</button>';
            tActionDrag = '<span class="file-drag-handle {dragClass}" title="{dragTitle}">{dragIcon}</span>';
            tIndicator = '<div class="file-upload-indicator" title="{indicatorTitle}">{indicator}</div>';
            tTagBef = '<div class="file-preview-frame {frameClass}" id="{previewId}" data-fileindex="{fileindex}"' +
                ' data-fileid="{fileid}" data-template="{template}"';
            tTagBef1 = tTagBef + '><div class="-content">\n';
            tTagBef2 = tTagBef + ' title="{caption}"><div class="-content">\n';
            tTagAft = '</div>{footer}\n</div>\n';
            tGeneric = '{content}\n';
            tStyle = ' {style}';
            ...
            $.each(self.previewContentTemplates, function (key, value) {
                if ($h.isEmpty(self.previewTemplates[key])) {
                    ...
                    if (self._isPdfRendered() && key === 'pdf') {
                        tagBef = tagBef.replace('-content', '-content kv-pdf-rendered');
                    }
        ...
        _initPreviewImageOrientations: function () {
            ...
            self.getFrames('.file-preview-initial').each(function () {
                ...
                if (config && config.exif && config.exif.Orientation) {
                    id = $thumb.attr('id');
                    $img = $thumb.find('>.-content img');
                    $zoomImg = self._getZoom(id, ' >.-content img');
        ...
        _setZoomContent: function ($frame, animate) {
            ...
            $content = $zoomPreview.find('.-content');
        ...
        _zoomSlideShow: function (dir, previewId) {
            ...
            for (i = 0; i < len; i++) {
                $thumb = $(thumbsData[i]);
                if ($thumb && $thumb.length && $thumb.find('.-zoom:visible').length) {
        ...
        _initZoomButton: function () {
            var self = this;
            self.$preview.find('.-zoom').each(function () {
                var $el = $(this);
        ...
        _initSuccessThumbs: function () {
            ...
            self._getThumbs($h.FRAMES + '.file-preview-success').each(function () {
                var $thumb = $(this), $remove = $thumb.find('.-remove');
        ...
        _uploadSingle: function (i, id, isBatch) {
            ...
            if (self.showPreview) {
                ...
                $btnUpload = $thumb.find('.-upload');
                $btnDelete = $thumb.find('.-remove');
                $prog.show();
            }
        ...
        _uploadBatch: function () {
            ...
            fnBefore = function (jqXHR) {
                ...
                if (self.showPreview) {
                    self._getThumbs().each(function () {
                        var $thumb = $(this), $btnUpload = $thumb.find('.-upload'),
                            $btnDelete = $thumb.find('.-remove');
                        if (!$thumb.hasClass('file-preview-success')) {
                            self._setThumbStatus($thumb, 'Loading');
                            $h.addCss($thumb, 'file-uploading');
                ...
                if ($h.isEmpty(data) || $h.isEmpty(data.error)) {
                    ...
                    if (self.showPreview) {
                        $thumbs.each(function () {
                            ...
                            self._setThumbStatus($thumb, 'Success');
                            $thumb.removeClass('file-uploading');
                            $thumb.find('.-upload').hide().removeAttr('disabled');
                        });
                    ...
                    if (self.showPreview) {
                        $thumbs.each(function () {
                            var $thumb = $(this);
                            $thumb.removeClass('file-uploading');
                            $thumb.find('.-upload').removeAttr('disabled');
                            $thumb.find('.-remove').removeAttr('disabled');
                            if (keys.length === 0 || $.inArray(key, keys) !== -1) {
                                self._setPreviewError($thumb, true);
                                if (!self.retryErrorUploads) {
                                    $thumb.find('.-upload').hide();
                                    self.fileManager.remove($thumb);
                                }
                            } else {
                                $thumb.find('.-upload').hide();
            ...
            fnError = function (jqXHR, textStatus, errorThrown) {
                ...
                self._getThumbs(' .-upload').removeAttr('disabled');
                self._getThumbs(' .-delete').removeAttr('disabled');
                self._setProgress(101, self.$progress, self.msgAjaxProgressError.replace('{operation}', op));
            };
            var ctr = 0;
        ...
        _initFileActions: function () {
            ...
            self.getFrames(' .-remove').each(function () {
            ...
            self.getFrames(' .-upload').each(function () {
                var $el = $(this);
                self._handler($el, 'click', function () {
        ...
        _initPreviewActions: function () {
            var self = this, $preview = self.$preview, deleteExtraData = self.deleteExtraData || {},
                btnRemove = $h.FRAMES + ' .-remove', settings = self.fileActionSettings,
        ...
        _setThumbStatus: function ($thumb, status) {
            ...
            if (status === 'Error' && !self.retryErrorUploads) {
                $thumb.find('.-upload').attr('disabled', true);
            }
        },
        ...
        _refreshUploadButton: function ($thumb) {
            var self = this, $btn = $thumb.find('.-upload'), cfg = self.fileActionSettings,
        ...
        setImageOrientation: function ($img, $zoomImg, value, $thumb) {
            ...
            if (zoomOnly) {
                ...
                $div = $(document.createElement('div')).appendTo($thumb.find('.-content'));
            ...
            $img.off(ev).on(ev, function () {
                if (isHidden) {
                    ...
                    $thumb.find('.-content').css('visibility', 'hidden');
                }
                var img = $img.get(0), zoomImg = $zoomImg && $zoomImg.length ? $zoomImg.get(0) : null,
                    h = img.offsetHeight, w = img.offsetWidth, r = $h.getRotation(value);
                if (isHidden) {
                    $thumb.find('.-content').css('visibility', 'visible');
        ...
        _renderMain: function () {
            var self = this,
                ...
                caption = self.captionTemplate.setTokens({'class': css + ' input-caption'});
            ...
            if (self.showPreview) {
                self._getThumbs().each(function () {
                    ...
                    if (!self.fileManager.getFile(fileId)) {
                        $thumb.find('.-remove').removeClass('disabled').removeAttr('disabled');
                    }
                });
            }
        ...
        cancel: function () {
            ...
            self._getThumbs().each(function () {
                ...
                $prog.hide();
                if (!self.fileManager.getFile(fileId)) {
                    $thumb.find('.-upload').removeClass('disabled').removeAttr('disabled');
                    $thumb.find('.-remove').removeClass('disabled').removeAttr('disabled');
                }
                self.unlock();
            });
        ...
        disable: function () {
            ...
            $container.find('.input-caption').addClass('file-caption-disabled');
        ...
        enable: function () {
            ...
            $container.removeClass('is-locked');
            $container.find('.input-caption').removeClass('file-caption-disabled');
            $container.find('.fileinput-remove, .fileinput-upload, .file-preview-frame button')
                .removeAttr('disabled');
